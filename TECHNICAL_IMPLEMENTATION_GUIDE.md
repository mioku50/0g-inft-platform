...
### Usage Examples from Real Implementation

#### Example 1: Express Proxy Server (from inference-server.ts)
```typescript
...
app.post('/v1/chat/completions', async (req, res) => {
    ...
    // Handle streaming and caching for verification
    if (stream) {
        // Stream response and cache for later verification
    } else {
        const data = await response.json();
        cache.setItem(data.id, data.choices[0].message.content, 5 * 60 * 1000);
        res.json(data);
    }
});
...
app.post('/v1/verify', async (req, res) => {
    const { id } = req.body;
    const content = cache.getItem(id);
...
#### Example 2: Complete Workflow Demo (from demo-compute-flow.ts)
```typescript
async function demonstrateCompleteFlow() {
    console.log("🚀 Starting 0G Compute Network Flow Demo");
    
    // Step 1: Initialize wallet and broker
    const provider = new ethers.JsonRpcProvider("https://evmrpc-testnet.0g.ai");
    const wallet = new ethers.Wallet(process.env.PRIVATE_KEY!, provider);
    const broker = await createZGComputeNetworkBroker(wallet);
    
    // Step 2: Setup ledger account
    try {
    ...
    // Step 3: List and select services
    const services = await broker.inference.listService();
    ...
    const headers = await broker.inference.getRequestHeaders(selectedProvider, "Hello");
    ...
    // Step 6: Process response and verify
    const isValid = await broker.inference.processResponse(
        selectedProvider,
    ...
    console.log(`✅ Query completed. Response valid: ${isValid}`);
}
```
